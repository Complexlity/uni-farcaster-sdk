...
//Initialize the sdk with either neynar api key or airstack api key
const sdkInstance = new uniFarcasterSdk({
  neynarApiKey: "NEYNAR_API_DOCS",
  airstackApiKey: "your-airstack-api-key",
});
//NOTE: You only need one of them but you can also have both.

//Start Querying the farcaster network
const { data, error } = await sdkInstance.getUserByFid(213144, 11124);
```
